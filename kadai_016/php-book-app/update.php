<?php
$dsn = 'mysql:dbname=php_book_app;host=localhost;charet=utf8mb4';
$user = 'root';
$password = '';

// submitパラメータの値が存在するとき（「更新」ボタンを押した時）の処理
...
if (isset($_GET['id'])) {
    try {
        ...
        // SQL文の実行結果を配列で取得
        // 一つのレコードのみ取得したい場合、fetch()メソッドを使えばカラム名がキーになった1次元配列を取得できる
        $book = $stmt_select_book->fetch(PDO::FETCH_ASSOC);

        // idパラメータの値と同じidのデータが存在しない場合はエラーメッセージを表示して処理を修了する。
        // 補足: fetch()メソッドは実行結果が取得できなかった場合にFALSEを返す
        if ($book === FALSE) {
            exit('idパラメータの値が不正です。');
        }

        // genreテーブルからgenre_codeカラムのデータを取得するためのSQL文を変数$sql_select_genre_codesに代入する
        ...
        // SQL文の実行結果を配列で取得する
        // 補足:PDO::FETCH_COLUMNは一つのカラムの値を一次元配列で取得する
                ...
                <div>
                    ...
                    <select id="genre_code" name="genre_code" required>
                        ...
                        foreach ($genre_codes as $genre_code) {
                            // もし変数$genre_codeが書籍のジャンルコードの値と一致していれば、selected属性を付けて初期値にする
